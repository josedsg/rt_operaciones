...
export async function getDevicesUsedData(
  ...
  const total = data.pedidosPorEstado.reduce((acc, p) => acc + p.count, 0);

  return data.pedidosPorEstado.map(p => ({
    name: p.name,
    percentage: total > 0 ? p.count / total : 0,
    amount: p.count
  }));
}
...
export async function getPaymentsOverviewData(
  ...
  return {
    received: data.monthlySales,
    due: data.monthlySales.map(m => ({ x: m.x, y: 0 })) // Deuda no implementada aún
  };
}
