...
import { createAgenciaAction, updateAgenciaAction } from "@/actions/agencias";
...
export function AgenciaForm({ isOpen, onClose, onSuccess, editingAgencia }: AgenciaFormProps) {
    ...
    useEffect(() => {
        if (editingAgencia) {
            setNombre(editingAgencia.nombre);
            setDescripcion(editingAgencia.descripcion || "");
        } else {
            setNombre("");
            setDescripcion("");
        }
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (editingAgencia) {
                res = await updateAgenciaAction(editingAgencia.id, data);
            } else {
                res = await createAgenciaAction(data);
            }
                ...
                <form onSubmit={handleSubmit}>
                    ...
                    <div className="mb-6">
                        ...
                        <textarea
                            ...
                            className="w-full rounded border border-stroke bg-white py-3 px-5 outline-none focus:border-primary dark:border-strokedark dark:bg-meta-4 focus:ring-1 focus:ring-primary"
                            placeholder="Descripción opcional (dirección, contacto, etc.)"
                        ></textarea>
                    </div>
