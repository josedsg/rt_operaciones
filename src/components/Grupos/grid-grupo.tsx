...
import { getGruposAction, deleteGrupoAction } from "@/actions/grupos";

export function GridGrupo() {
    ...
    const fetchGrupos = async () => {
        setLoading(true);
        try {
            const data = await getGruposAction();
            setGrupos(data);
        } catch (error) {
                    ...
                    <table className="w-full table-auto">
                        <thead>
                            <tr className="bg-gray-2 text-left dark:bg-meta-4">
                                ...
                                <th className="min-w-[200px] py-4 px-4 font-medium text-black dark:text-white">Descripción</th>
                            ...
                            {filteredGrupos.map((grupo) => (
                                <tr key={grupo.id}>
                                    <td className="border-b border-[#eee] py-5 px-4 dark:border-strokedark">
                                        <h5 className="font-medium text-black dark:text-white">{grupo.nombre}</h5>
                                    </td>
                                    <td className="border-b border-[#eee] py-5 px-4 dark:border-strokedark">
                                        <p className="text-black dark:text-white truncate max-w-xs" title={grupo.descripcion}>
                                            {grupo.descripcion}
                                        </p>
                                    </td>
                                    <td className="border-b border-[#eee] py-5 px-4 dark:border-strokedark">
                                    ...
                                    <td className="border-b border-[#eee] py-5 px-4 dark:border-strokedark">
                                        <div className="flex items-center space-x-3.5">
                                            <button onClick={() => handleEdit(grupo)} className="hover:text-primary">
                                                <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7" /><path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z" /></svg>
                                            </button>
                                            <button onClick={() => handleDelete(grupo.id)} className="hover:text-meta-1">
