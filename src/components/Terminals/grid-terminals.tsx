...
import { getTerminalsAction, deleteTerminalAction } from "@/actions/terminals";
import { TerminalForm } from "./terminal-form";

export default function GridTerminals() {
    ...
    const handleDelete = async (id: number) => {
        if (confirm("¿Estás seguro de eliminar esta terminal?")) {
            const res = await deleteTerminalAction(id);
            if (res.success) {
                fetchTerminals();
            } else {
                alert(res.error);
                            ...
                            {terminals.map((terminal) => (
                                <tr key={terminal.id}>
                                    <td className="border-b border-[#eee] py-5 px-4 pl-9 dark:border-strokedark xl:pl-11">
                                        <p className="text-black dark:text-white">{terminal.id}</p>
                                    </td>
                                    <td className="border-b border-[#eee] py-5 px-4 dark:border-strokedark">
                                        <p className="text-black dark:text-white">{terminal.nombre}</p>
                                    </td>
                                    <td className="border-b border-[#eee] py-5 px-4 dark:border-strokedark">
                                        <p className="text-black dark:text-white">{terminal.descripcion || "-"}</p>
                                    </td>
                                    <td className="border-b border-[#eee] py-5 px-4 dark:border-strokedark">
                                        <div className="flex items-center space-x-3.5">
                                            <button onClick={() => openEdit(terminal)} className="hover:text-primary">
                                                Editar
                                            </button>
                                            <button onClick={() => handleDelete(terminal.id)} className="hover:text-danger text-red-500">
                                                Eliminar
                                            </button>
                                        </div>
                                    </td>
