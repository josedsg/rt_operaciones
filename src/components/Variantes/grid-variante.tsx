...
import { getVariantesAction, deleteVarianteAction } from "@/actions/variantes";

export function GridVariante() {
    ...
    const fetchVariantes = useCallback(async () => { // Wrapped in useCallback
        setLoading(true);
        try {
            const response = await getVariantesAction({
                page: currentPage,
                limit: 10,
    ...
    }, [currentPage, debouncedSearch]); // Dependencies for fetchVariantes

    useEffect(() => {
        fetchVariantes();
    }, [fetchVariantes]); // Added fetchVariantes as dependency
                            ...
                            {variantes.map((variante) => (
                                <tr key={variante.id}>
                                    <td className="border-b border-[#eee] py-5 px-4 dark:border-strokedark">
                                        <p className="text-black dark:text-white">#{variante.id}</p>
                                    </td>
                                    <td className="border-b border-[#eee] py-5 px-4 dark:border-strokedark">
                                        <h5 className="font-medium text-black dark:text-white">{variante.nombre}</h5>
                                    </td>
                                    <td className="border-b border-[#eee] py-5 px-4 dark:border-strokedark">
                                        <p className="text-sm text-black dark:text-white">
                                            {variante.fecha_creacion ? new Date(variante.fecha_creacion).toLocaleDateString() : '-'}
                                        </p>
                                    </td>
                                    <td className="border-b border-[#eee] py-5 px-4 dark:border-strokedark">
                                        <div className="flex items-center space-x-3.5">
                                            <button onClick={() => handleEdit(variante)} className="hover:text-primary">
                                                <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7" /><path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z" /></svg>
                                            </button>
                                            <button onClick={() => handleDelete(variante.id)} className="hover:text-meta-1">
